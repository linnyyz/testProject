package com.yizhi.oa.controller;
...
import com.yizhi.oa.domain.Response;
import com.yizhi.oa.service.NotifyRecordService;
...
public class NotifyController extends BaseController {
	...
	public R save(NotifyDO notify) {
		...
		if (notifyService.save(notify) > 0) {
			...
			String[] typename={"","会议通告","奖惩通告","活动通告"};
			for (Long userId : userIds) {
				template.convertAndSend("/topic/getResponse/"+userId, new Response("【"+typename[Integer.parseInt(notify.getType())]+"】"+notify.getTitle() ));
			}
			return R.ok();
		}
		return R.error();
