...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    //数据的异步请求
    ,req: view.req
    ,requestFiles: view.requestFiles
    ,uploadFiles: view.uploadFiles
    //时间范围分割
    ,timeDec: function(rangeTime,separator){
        separator = separator ? separator : '~';
        var timeArr = rangeTime.split(separator), arr=[];
    ...
    //合并时间范围
    ,timeMerge: function(s_time,e_tiem,separator){
      separator = separator ? separator : ' ~ ';
      var arr = [s_time,e_tiem];
      return arr.join(separator);
    }
	,selectRender: view.selectRender
  ...
  ,menuMerge: function(list,index){
		var that = this;
		var arr = new Array();
		$.each(list,function(i,v){
			if(v.parentId == index){
	            v.children = that.menuMerge(list,v.menuId);
	            arr.push(v);
	        }
		})
		return arr;
  },
  reloadCurrTabTable: function(id){
    id = id ? id : 'table_test';
    ...
    iframe[0].contentWindow.layui.table.reload(id);
  }
  ,mergeCols:function(data,colNameArr,indexArr){
    var mergeIndex = 0;//定位需要添加合并属性的行数
    var mark = 1; //这里涉及到简单的运算，mark是计算每次需要合并的格子数
    var columsName = colNameArr;   //['id','sign'];//需要合并的列名称
    var columsIndex = indexArr;   //[0,3];//需要合并的列索引值
  ...
  , renderTable: function(options){
    ...
    var $thead = $('<thead>',{class:'layui-anim layui-anim-upbit'}), $tbody = $('<tbody>',{class:'layui-anim layui-anim-upbit'});
    // var $loading = $(['<div class="layui-table-init">','<i class="layui-icon layui-icon-loading layui-icon"></i>','</div>'].join(''));
    // $(elem).append($loading);
    ...
    admin.req({
      ...
      success:function(res){
        var renderData = typeof parseData == 'function' ? parseData(res) : res.data;
        var $thr = $('<tr>');
        $.each(cols,function(i,v){
          var $th = $('<th>').text(v.title);
          $thr.append($th);
        });
        $thead.append($thr);
        $.each(renderData,function(i,v){
          if(i < limit){
            var $tr = $('<tr>');
            $.each(cols,function(k,e){
              var cont = typeof e.template == 'function'? e.template(i,v) : v[e.field];
              var $td = $('<td>').html(cont);
              $tr.append($td);
            });
            $tbody.append($tr);
    ...
    var data = options.data || [],
        ...
        $.each(filter,function(i,v){
          var curr = data.filter(function(item){
                return item[field] == v ? true : false;
              });
          if(curr.length > 0){
            var index = data.indexOf(curr[0]);
            data.splice(index, 1);
          }
        });
