...
(function() {
    ...
    //打开对话框(添加修改)
       ...
       //打开对话框(自定义Button)
        openDialogWithBtn2: function(title,url,width,height,btn1,btn2, $table){
            var auto = true;//是否使用响应式，使用百分比时，应设置为false
            if(width.indexOf("%")>=0 || height.indexOf("%")>=0 ){
                auto =false;
            }
	   ...
	   openUserSelectDialog:function(isMultiSelect, yesFuc){
        ...
        /**自己的用户选择框**/
        openOwnerUserSelectDialog:function(isMultiSelect, yesFuc){
            top.layer.open({
                type: 2,
                area: ['900px', '560px'],
                title:"选择用户",
                auto:true,
                maxmin: true, //开启最大化最小化按钮
                content: ctx+"/sys/user/ownerUserSelect?isMultiSelect="+isMultiSelect,
                btn: ['确定', '关闭'],
                yes: function(index, layero){
                    var ids = layero.find("iframe")[0].contentWindow.getIdSelections();
                    var names = layero.find("iframe")[0].contentWindow.getNameSelections();
